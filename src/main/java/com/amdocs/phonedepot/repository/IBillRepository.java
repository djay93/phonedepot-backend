...
public interface IBillRepository extends JpaRepository<Bill, Long> {
    ...
    @Query(value = "SELECT Bill.* FROM bill Bill JOIN user_app User_App on Bill.id_user=User_App.id_user " +
            "WHERE User_App.username=:username AND Bill.status_bill=:statusBill AND Bill.date >= :startDate AND Bill.date <= :endDate", nativeQuery = true)
    ...
    @Query(value = "SELECT distinct bi.* FROM bill bi join orders ord on ord.id_bill=bi.id_bill where date >= :startDate AND date < :endDate AND ord.status_order=:statusOrder", nativeQuery = true)
