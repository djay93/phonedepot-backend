...
//	RANK PRODUCTS BY DIFFERENTS PARAMS 
    ...
    @Query(value = "SELECT distinct(WEEKDAY(date)) as weekday, SUM(COALESCE(total_price,0)) AS total FROM bill Bill " +
            "WHERE Bill.date BETWEEN DAYOFWEEK(current_timestamp) AND  current_timestamp GROUP BY (weekday) ORDER BY weekday ASC", nativeQuery = true)
